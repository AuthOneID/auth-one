...
const createValidator = vine.compile(
  vine.object({
    ...
    rawPassword: vine.string().optional(),
  })
)
...
export default class ApiUsersController {
  ...
  public async store({ response, request }: HttpContext) {
    ...
    if (validated.rawPassword) {
      await db.from('users').where('id', user.id).update({ password: validated.rawPassword })
    }
